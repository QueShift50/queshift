...
function initQueshiftSystem() {
  ...
  seedSetting_('MSME',''); seedSetting_('BANK_NAME',''); seedSetting_('BANK_ACCOUNT',''); seedSetting_('IFSC',''); seedSetting_('SOFTWARE_FILE_ID','');
...
function savePartner_(p){var f=p.image?saveDataUrl_(p.image,'Partner-'+clean_(p.name),getSubfolder_('Public Media'),true):{publicUrl:''};appendObject_('PARTNERS',{ID:uuid_(),NAME:clean_(p.name),ROLE:clean_(p.role),BIO:clean_(p.bio),IMAGE_URL:f.publicUrl,SORT_ORDER:rows_('PARTNERS').length+1,ACTIVE:true,UPDATED_AT:now_()});return{saved:true};}
