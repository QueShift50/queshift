...
/*
  TRIAL DOWNLOAD SECURITY
  -----------------------
  Restricted Google Drive file access is granted only after OTP verification.
  Access is automatically revoked after the configured window (default 120 minutes).
  This does NOT change the Queshift Desktop activation API or paid-subscription flow.
*/
var QS_DOWNLOAD_ACCESS_MINUTES = 12 * 60; // 12 hours for BOTH Trial and Paid downloads
var QS_HEADERS = {
...
function initQueshiftSystem() {
  ...
  seedSetting_('TRIAL_ACCESS_MINUTES','720');
...
function doGet(e) {
  try {
    ...
    else if (action === 'health') data = {status:'ok',build:'V18-POST-BRIDGE',time:new Date().toISOString()};
...
function doPost(e) {
  try {
    ...
    if (action === 'postHealth') data = {status:'ok',build:'V18-POST-BRIDGE',time:new Date().toISOString()};
else if (action === 'contact') data = contact_(payload);
...
// Queshift Desktop Activation Tool
  ...
  var identity = verifyCredential_(credential);
      if (action === 'saveProfile') data = saveProfile_(identity,payload);
        ...
        if (action === 'adminPaymentsFast') data = adminPaymentsFast_();
        else if (action === 'adminDashboard') data = adminDashboard_();
...
/* =========================
   QUESHIFT V13 ULTRAFAST CORE
   ========================= */
var QS_FAST_PUBLIC_CACHE_KEY = 'QS_PUBLIC_DATA_V13';
var QS_FAST_PUBLIC_SNAPSHOT_PREFIX = 'QS_PUBLIC_SNAPSHOT_V13';
var QS_FAST_ADMIN_CACHE_KEY = 'QS_ADMIN_DASH_V13';
var QS_FAST_BLOGS_CACHE_KEY = 'QS_PUBLIC_BLOGS_V13';
var QS_FAST_HELP_CACHE_KEY = 'QS_PUBLIC_HELP_V13';

function qsCacheGet_(key){
  try{
...
function qsCachePut_(key,obj,seconds){
  try{
    var raw=JSON.stringify(obj);
    if(raw.length<95000)CacheService.getScriptCache().put(key,raw,seconds||600);
  }catch(_){}
}

function qsChunkDelete_(prefix){
  try{
    var props=PropertiesService.getScriptProperties();
    var count=+(props.getProperty(prefix+'_COUNT')||0);
    var keys=[prefix+'_COUNT'];
    for(var i=0;i<count;i++)keys.push(prefix+'_'+i);
    props.deleteProperties(keys);
  }catch(_){}
}

function qsChunkPut_(prefix,obj){
  try{
    var raw=JSON.stringify(obj);
    var size=7000;
    var count=Math.ceil(raw.length/size);
    var values={};
    values[prefix+'_COUNT']=String(count);
    for(var i=0;i<count;i++)values[prefix+'_'+i]=raw.slice(i*size,(i+1)*size);
    qsChunkDelete_(prefix);
    PropertiesService.getScriptProperties().setProperties(values,false);
  }catch(_){}
}

function qsChunkGet_(prefix){
  try{
    var props=PropertiesService.getScriptProperties();
    var count=+(props.getProperty(prefix+'_COUNT')||0);
    ...
    for(var i=0;i<count;i++){
      var part=props.getProperty(prefix+'_'+i);
...
function clearPublicCache_(){
  try{
    ...
    cache.remove(QS_FAST_PUBLIC_CACHE_KEY);
    cache.remove(QS_FAST_BLOGS_CACHE_KEY);
    cache.remove(QS_FAST_HELP_CACHE_KEY);
  }catch(_){}
  qsChunkDelete_(QS_FAST_PUBLIC_SNAPSHOT_PREFIX);
}

function clearAdminCache_(){
  try{CacheService.getScriptCache().remove(QS_FAST_ADMIN_CACHE_KEY);}catch(_){}
}

function clearAllFastCaches_(){
  clearPublicCache_();
  clearAdminCache_();
}

function qsRowsFromValues_(name,values){
  var headers=QS_HEADERS[name]||[];
...
function qsRowsFallback_(names){
  var out={};
  names.forEach(function(n){out[n]=rows_(n);});
  return out;
}

function qsBatchRows_(names){
  try{
    var props=PropertiesService.getScriptProperties();
    var id=props.getProperty('SPREADSHEET_ID');
    if(!id)id=ss_().getId();

    var query=names.map(function(n){
      return 'ranges='+encodeURIComponent("'"+n+"'");
    }).join('&');

    var url='https://sheets.googleapis.com/v4/spreadsheets/'+encodeURIComponent(id)
    ...
    names.forEach(function(n,i){
      out[n]=qsRowsFromValues_(n,(ranges[i]&&ranges[i].values)||[]);
    });
    return out;
  }catch(_){
    return qsRowsFallback_(names);
...
function qsSettingsFromRows_(rows){
  var o={};
  (rows||[]).forEach(function(r){o[String(r.KEY||'')]=r.VALUE;});
  return o;
}

function trialPublicConfigFast_(settings){
  var s=settings||{};
  var ready=!!clean_(s.TRIAL_SOFTWARE_FILE_ID||s.TRIAL_DOWNLOAD_URL);
  return{
...
function qsBuildPublicFromBatch_(b){
  var settings=qsSettingsFromRows_(b.SETTINGS||[]);
  var gst=+settings.GST_RATE||18;
  ...
  return{
    ...
    phone1:settings.PHONE1||'9310907124',
    phone2:settings.PHONE2||'9310907125',
    email:settings.EMAIL||QS_ADMIN_EMAIL,
    ...
    plans:(b.PLANS||[]).filter(active_).map(function(r){
      return{code:r.CODE,name:r.NAME,price:+r.BASE_PRICE,gstRate:+r.GST_RATE||gst,days:+r.DAYS,active:bool_(r.ACTIVE)};
    }),
    banners:(b.BANNERS||[]).filter(active_).sort(sort_).map(function(r){
      return{id:r.ID,title:r.TITLE,imageUrl:publicMediaUrl_(r.IMAGE_URL),link:r.LINK,active:bool_(r.ACTIVE)};
    }),
    partners:(b.PARTNERS||[]).filter(active_).sort(function(a,b){
      var an=String(a.NAME||'').toLowerCase(),bn=String(b.NAME||'').toLowerCase();
      var ar=an==='ajay kumar'?1:an==='wasim raza'?2:(+a.SORT_ORDER||99);
      var br=bn==='ajay kumar'?1:bn==='wasim raza'?2:(+b.SORT_ORDER||99);
      return ar-br;
    }).map(function(r){
      return{id:r.ID,name:r.NAME,role:r.ROLE,bio:r.BIO,imageUrl:publicMediaUrl_(r.IMAGE_URL)};
    }),
    brands:(b.BRANDS||[]).filter(active_).sort(sort_).map(function(r){
      return{id:r.ID,name:r.NAME,imageUrl:publicMediaUrl_(r.IMAGE_URL),url:r.URL};
    }),
    videos:(b.VIDEOS||[]).filter(active_).map(function(r){
      return{id:r.ID,url:r.URL,videoId:r.VIDEO_ID,title:r.TITLE,description:r.DESCRIPTION,thumbnail:r.THUMBNAIL,featured:bool_(r.FEATURED),active:bool_(r.ACTIVE)};
    }),
    reviews:(b.REVIEWS||[]).filter(function(r){
      return String(r.STATUS||'').toUpperCase()==='APPROVED';
    }).slice().reverse().slice(0,20).map(function(r){
      return{id:r.ID,name:r.NAME,rating:+r.RATING||5,comment:r.COMMENT,reply:r.REPLY,status:r.STATUS};
    })
  };
}

function qsBuildPublicData_(){
  var names=['SETTINGS','PLANS','BANNERS','PARTNERS','BRANDS','VIDEOS','REVIEWS'];
  return qsBuildPublicFromBatch_(qsBatchRows_(names));
}

function warmQueshiftFastCache(){
  clearAllFastCaches_();
  var pub=qsBuildPublicData_();
  qsCachePut_(QS_FAST_PUBLIC_CACHE_KEY,pub,1800);
  qsChunkPut_(QS_FAST_PUBLIC_SNAPSHOT_PREFIX,pub);
  try{publicBlogs_();}catch(_){}
  try{publicHelpArticles_();}catch(_){}
  return{
    ready:true,
    build:'V13-ULTRAFAST',
    publicPartners:(pub.partners||[]).length,
    publicBanners:(pub.banners||[]).length,
    publicBrands:(pub.brands||[]).length,
    publicReviews:(pub.reviews||[]).length
  };
}


function publicData_(){
  var cached=qsCacheGet_(QS_FAST_PUBLIC_CACHE_KEY);
  if(cached)return cached;

  var snapshot=qsChunkGet_(QS_FAST_PUBLIC_SNAPSHOT_PREFIX);
  if(snapshot){
    qsCachePut_(QS_FAST_PUBLIC_CACHE_KEY,snapshot,1800);
    return snapshot;
  }

  var data=qsBuildPublicData_();
  qsCachePut_(QS_FAST_PUBLIC_CACHE_KEY,data,1800);
  qsChunkPut_(QS_FAST_PUBLIC_SNAPSHOT_PREFIX,data);
  return data;
}
...
function publicBlogs_(){
  var cached=qsCacheGet_(QS_FAST_BLOGS_CACHE_KEY);
  if(cached)return cached;
  ...
  qsCachePut_(QS_FAST_BLOGS_CACHE_KEY,out,1800);
  return out;
}
...
function publicHelpArticles_(){
  var cached=qsCacheGet_(QS_FAST_HELP_CACHE_KEY);
  if(cached)return cached;

  var out=rows_('HELP_ARTICLES')
    .filter(helpIsPublished_)
    .sort(function(a,b){
      var ad=new Date(a.PUBLISHED_AT||a.UPDATED_AT||0).getTime()||0;
      var bd=new Date(b.PUBLISHED_AT||b.UPDATED_AT||0).getTime()||0;
      return bd-ad;
  ...
  qsCachePut_(QS_FAST_HELP_CACHE_KEY,out,1800);
  return out;
}
...
function saveHelpArticle_(p){
  ...
  clearAllFastCaches_();return{saved:true,slug:slug};
}
...
function userDashboard_(id) {
  var u=findOne_('USERS','EMAIL',id.email)||{},
      sub=latest_('SUBSCRIPTIONS','USER_EMAIL',id.email),
      pay=latest_('PAYMENTS','USER_EMAIL',id.email),
      downloadUrl='';

  /*
    PAID DOWNLOAD SECURITY
    ----------------------
    Every time an ACTIVE paid customer opens the dashboard, give that exact
    Google account a fresh 12-hour Drive viewer window WITHOUT sending
    Google's "item shared" email. After the window expires, saved Drive links
    stop opening until the customer logs in again.
  */
  if(sub && String(sub.STATUS||'').toUpperCase()==='ACTIVE'){
    var paidFileId=clean_(getSetting_('SOFTWARE_FILE_ID'));
    if(paidFileId){
      try{
        downloadUrl=grantTimedDriveViewer_(id.email,paidFileId,'PAID');
      }catch(_){
        downloadUrl='';
      }
    }else{
      downloadUrl=sub.DOWNLOAD_URL||'';
    }
  }
  ...
  return{
    ...
    subscription:sub?{
      plan:sub.PLAN,
      start:formatDate_(sub.START_DATE),
      expiry:formatDate_(sub.EXPIRY_DATE),
      status:sub.STATUS
    }:null,
    latestPayment:pay?{orderId:pay.ORDER_ID,status:pay.STATUS}:null,
    downloadUrl:downloadUrl,
    invoices:rows_('INVOICES')
      .filter(function(r){return r.USER_EMAIL===id.email;})
      .map(function(r){
        return{
...
function paymentAttempt_(id,p) {
  ...
  clearAdminCache_();return{orderId:orderId,status:'PENDING',total:total,baseAmount:base,originalBase:originalBase,discountPercent:discountPercent,coupon:getSetting_('FIRST_BOOKING_COUPON')||'FIRST20'};
}



/*
  FAST PAYMENT APPROVAL FEED
  --------------------------
  This intentionally reads only PAYMENTS + PLANS so the Payments tab can open
  immediately even if the full Admin Dashboard contains large blogs/help HTML.
*/
function adminPaymentsFast_(){
  var b=qsBatchRows_(['PAYMENTS','PLANS']);
  var payments=b.PAYMENTS||[],plans=b.PLANS||[];

  return{
    build:'V17-ADMIN-PAYMENT-FAST',
    plans:plans.map(function(r){
      return{
        code:r.CODE,
        name:r.NAME,
        price:+r.BASE_PRICE,
    ...
    payments:payments.slice().reverse().map(function(p){
      return{
        orderId:p.ORDER_ID,
        name:p.NAME,
        phone:p.PHONE,
    ...
    pendingPayments:payments.filter(function(p){
      return String(p.STATUS||'').toUpperCase()==='PENDING';
    }).length
  };
}

function adminDashboard_(){
  var cached=qsCacheGet_(QS_FAST_ADMIN_CACHE_KEY);
  if(cached)return cached;

  var names=[
    'SETTINGS','USERS','PAYMENTS','SUBSCRIPTIONS','BLOGS','HELP_ARTICLES',
    'INVOICES','ENQUIRIES','TRIAL_LEADS','PLANS','BANNERS','PARTNERS',
    'BRANDS','VIDEOS','REVIEWS','COMMENTS'
  ];
  var b=qsBatchRows_(names);
  var settings=qsSettingsFromRows_(b.SETTINGS||[]);
  var users=b.USERS||[],payments=b.PAYMENTS||[],subs=b.SUBSCRIPTIONS||[],
      blogs=b.BLOGS||[],helpArticles=b.HELP_ARTICLES||[],invoices=b.INVOICES||[],
      enquiries=b.ENQUIRIES||[],trialLeads=b.TRIAL_LEADS||[],
      reviews=b.REVIEWS||[],comments=b.COMMENTS||[];

  var pub=qsBuildPublicFromBatch_(b);
  var latestSubByEmail={};
  subs.forEach(function(s){
    latestSubByEmail[String(s.USER_EMAIL||'').toLowerCase()]=s;
  });
  ...
  var out={
    ...
    social:parseJson_(settings.SOCIAL_JSON,'{}'),
    plans:(b.PLANS||[]).map(function(r){
      return{code:r.CODE,name:r.NAME,price:+r.BASE_PRICE,gstRate:+r.GST_RATE||(+settings.GST_RATE||18),days:+r.DAYS,active:bool_(r.ACTIVE)};
    }),
    counts:{
      ...
      trialRequests:trialLeads.length,
      verifiedTrials:trialLeads.filter(function(t){return bool_(t.VERIFIED);}).length
    },
    banners:pub.banners,
    partners:pub.partners,
    brands:pub.brands,
    videos:pub.videos,
    blogs:blogs.map(function(x){return{id:x.ID,title:x.TITLE,slug:x.SLUG,status:x.STATUS};}),
    helpArticles:helpArticles.map(function(h){
      return{id:h.ID,slug:h.SLUG,category:h.CATEGORY,title:h.TITLE,summary:h.SUMMARY,html:h.HTML,
        metaTitle:h.META_TITLE,metaDescription:h.META_DESCRIPTION,keywords:h.KEYWORDS,language:h.LANGUAGE,status:h.STATUS};
    }),
    payments:payments.slice().reverse().map(function(p){
      return{orderId:p.ORDER_ID,name:p.NAME,phone:p.PHONE,state:p.STATE,plan:p.PLAN,amount:p.TOTAL_AMOUNT,screenshotUrl:p.SCREENSHOT_URL,status:p.STATUS};
    }),
    reviews:reviews.concat(comments.map(function(c){
      return{ID:c.ID,NAME:c.NAME,RATING:0,COMMENT:'Blog '+c.BLOG_SLUG+': '+c.COMMENT,STATUS:c.STATUS,REPLY:c.REPLY};
    })).map(function(r){
      return{id:r.ID,name:r.NAME,rating:r.RATING,comment:r.COMMENT,status:r.STATUS,reply:r.REPLY};
    }),
    customers:users.map(function(u){
      var s=latestSubByEmail[String(u.EMAIL||'').toLowerCase()];
      return{name:u.NAME,email:u.EMAIL,phone:u.PHONE,state:u.STATE,plan:s?s.PLAN:''};
    }),
    invoices:invoices.slice().reverse().map(function(i){
      return{invoiceNumber:i.INVOICE_NUMBER,customer:i.CUSTOMER_NAME,total:i.TOTAL,pdfUrl:i.PDF_URL};
    }),
    enquiries:enquiries.slice().reverse().map(function(q){
      return{id:q.ID,name:q.NAME,email:q.EMAIL,phone:q.PHONE,company:q.COMPANY,message:q.MESSAGE,status:q.STATUS,reply:q.REPLY,createdAt:formatDateTime_(q.CREATED_AT)};
    }),
    trialLeads:trialLeads.slice().reverse().map(function(t){
      return{id:t.ID,name:t.NAME,email:t.EMAIL,phone:t.PHONE,company:t.COMPANY,state:t.STATE,
        verified:bool_(t.VERIFIED),verifiedAt:formatDateTime_(t.VERIFIED_AT),
        downloads:+t.DOWNLOAD_COUNT||0,createdAt:formatDateTime_(t.CREATED_AT)};
    })
  };

  qsCachePut_(QS_FAST_ADMIN_CACHE_KEY,out,30);
  return out;
}
...
function saveSettings_(p) {
  ...
  clearAllFastCaches_();return{saved:true};
}
function saveSocial_(p){var social={},standard=['youtube','instagram','facebook','linkedin','twitter','awtaxation','whatsapp'];standard.forEach(function(k){social[k]=clean_(p[k]);});social.custom=[];[1,2].forEach(function(i){if(p['customUrl'+i])social.custom.push({label:clean_(p['customLabel'+i])||'Link',icon:clean_(p['customLabel'+i]).slice(0,2)||'+',url:clean_(p['customUrl'+i])});});setSetting_('SOCIAL_JSON',JSON.stringify(social));clearAllFastCaches_();return{saved:true};}
function savePlan_(p){
  ...
  clearAllFastCaches_();return{saved:true,code:code};
}
function saveSoftwareFile_(p){
  ...
  var file=DriveApp.getFileById(id);setSetting_('SOFTWARE_FILE_ID',id);clearAdminCache_();return{saved:true,fileId:id,name:file.getName(),url:file.getUrl()};
}
function saveBanner_(p){var f=saveDataUrl_(p.image,'Banner-'+Date.now(),getSubfolder_('Public Media'),true);appendObject_('BANNERS',{ID:uuid_(),TITLE:clean_(p.title),IMAGE_URL:f.publicUrl,LINK:clean_(p.link),SORT_ORDER:+p.sortOrder||1,ACTIVE:bool_(p.active),UPDATED_AT:now_()});clearAllFastCaches_();return{saved:true};}
...
function saveBrand_(p){var f=p.image?saveDataUrl_(p.image,'Brand-'+clean_(p.name),getSubfolder_('Public Media'),true):{publicUrl:''};appendObject_('BRANDS',{ID:uuid_(),NAME:clean_(p.name),IMAGE_URL:f.publicUrl,URL:clean_(p.url),SORT_ORDER:+p.sortOrder||1,ACTIVE:true,UPDATED_AT:now_()});clearAllFastCaches_();return{saved:true};}
function saveVideo_(p){var id=parseVideoId_(p.url);if(!id)throw new Error('Enter a valid YouTube video link.');var meta=youtubeMeta_(id),featured=bool_(p.featured);if(featured)rowsWithNumbers_('VIDEOS').forEach(function(x){if(bool_(x.obj.FEATURED)){sheet_('VIDEOS').getRange(x.row,7).setValue(false);}});appendObject_('VIDEOS',{ID:uuid_(),URL:p.url,VIDEO_ID:id,TITLE:clean_(p.title)||meta.title,DESCRIPTION:clean_(p.description)||meta.description,THUMBNAIL:meta.thumbnail,FEATURED:featured,ACTIVE:bool_(p.active),UPDATED_AT:now_()});clearAllFastCaches_();return{saved:true};}
function saveBlog_(p){var slug=slugify_(p.slug||p.title),existing=findOne_('BLOGS','SLUG',slug),imageUrl=existing?existing.IMAGE_URL:'';if(p.image)imageUrl=saveDataUrl_(p.image,'Blog-'+slug,getSubfolder_('Public Media'),true).publicUrl;upsertObject_('BLOGS','SLUG',slug,{ID:existing?existing.ID:uuid_(),SLUG:slug,TITLE:clean_(p.title),SUMMARY:clean_(p.summary),HTML:sanitizeHtml_(p.html),IMAGE_URL:imageUrl,META_TITLE:clean_(p.metaTitle)||clean_(p.title),META_DESCRIPTION:clean_(p.metaDescription)||clean_(p.summary),KEYWORDS:clean_(p.keywords),TAGS:clean_(p.tags),STATUS:p.status==='DRAFT'?'DRAFT':'PUBLISHED',PUBLISHED_AT:existing?existing.PUBLISHED_AT:now_(),UPDATED_AT:now_()});clearAllFastCaches_();return{saved:true,slug:slug};}
function deleteContent_(p){var allowed=['BANNERS','PARTNERS','BRANDS','VIDEOS','BLOGS','HELP_ARTICLES','REVIEWS','COMMENTS'];if(allowed.indexOf(p.type)<0)throw new Error('Invalid content type.');deleteBy_((p.type==='COMMENTS'?'COMMENTS':p.type),'ID',p.id);clearAllFastCaches_();return{deleted:true};}
...
function approvePayment_(admin,p) {
  ...
  var start=new Date(),expiry=new Date(start.getTime()+days*86400000),softwareId=getSetting_('SOFTWARE_FILE_ID'),downloadUrl='';if(softwareId){downloadUrl=grantTimedDriveViewer_(payment.USER_EMAIL,softwareId,'PAID');}
...
function submitComment_(id,p){var u=findOne_('USERS','EMAIL',id.email)||{};appendObject_('COMMENTS',{ID:uuid_(),BLOG_SLUG:clean_(p.slug),USER_EMAIL:id.email,NAME:u.NAME||id.name||id.email,COMMENT:clean_(p.comment),STATUS:'PENDING',REPLY:'',CREATED_AT:now_(),UPDATED_AT:now_()});clearAdminCache_();return{submitted:true};}
function submitReview_(id,p){var u=findOne_('USERS','EMAIL',id.email)||{},rating=Math.max(1,Math.min(5,+p.rating||0));appendObject_('REVIEWS',{ID:uuid_(),USER_EMAIL:id.email,NAME:u.NAME||id.name||id.email,RATING:rating,COMMENT:clean_(p.comment),STATUS:'PENDING',REPLY:'',CREATED_AT:now_(),UPDATED_AT:now_()});clearAdminCache_();return{submitted:true};}
function reviewAction_(p){var targets=['REVIEWS','COMMENTS'],found;targets.some(function(s){var r=findRow_(s,'ID',p.id);if(r){found={sheet:s,row:r.row};return true;}});if(!found)throw new Error('Review/comment not found.');var sh=sheet_(found.sheet),headers=QS_HEADERS[found.sheet],statusCol=headers.indexOf('STATUS')+1,replyCol=headers.indexOf('REPLY')+1;if(p.task==='APPROVE')sh.getRange(found.row,statusCol).setValue('APPROVED');else if(p.task==='REPLY'){sh.getRange(found.row,replyCol).setValue(clean_(p.reply));sh.getRange(found.row,statusCol).setValue('APPROVED');}clearAllFastCaches_();return{saved:true};}
function contact_(p){
  p=p||{};
...
function enquiryAction_(p){
  ...
  clearAdminCache_();return{saved:true,status:status};
}


function trialPublicConfig_(){
  return trialPublicConfigFast_(settingsObject_());
}
...
function downloadAccessMinutes_(){
  return QS_DOWNLOAD_ACCESS_MINUTES; // fixed 12 hours
}

function downloadAccessPropertyKey_(email,fileId){
  var raw=String(email||'').trim().toLowerCase()+'|'+String(fileId||'').trim();
  ...
  return 'QS_DOWNLOAD_ACCESS_'+hex;
}
...
function findDrivePermissionForEmail_(fileId,email){
  var url='https://www.googleapis.com/drive/v3/files/'+encodeURIComponent(fileId)
    +'/permissions?supportsAllDrives=true&fields=permissions(id,type,role,emailAddress)';
  var res=driveApiFetch_(url,{method:'get'});
  var code=res.getResponseCode();
  if(code<200||code>=300)return null;

  var data=parseJson_(res.getContentText(),'{}');
  var target=String(email||'').trim().toLowerCase();

  return (data.permissions||[]).filter(function(p){
    return String(p.type||'').toLowerCase()==='user'
      && String(p.emailAddress||'').trim().toLowerCase()===target;
  })[0]||null;
}

function createSilentDriveViewer_(fileId,email){
  /*
    Uses Google Drive API instead of DriveApp.addViewer().
    sendNotificationEmail=false prevents Google's automatic
    "an item was shared with you" email.
  */
  var normalized=String(email||'').trim().toLowerCase();
  if(!isValidEmail_(normalized))throw new Error('Valid Google email is required.');

  var existing=findDrivePermissionForEmail_(fileId,normalized);
  ...
  if(code<200||code>=300){
    throw new Error('Unable to grant secure Drive access. HTTP '+code);
  }

  var permission=parseJson_(res.getContentText(),'{}');
  if(!permission.id)throw new Error('Drive permission ID was not returned.');
  return permission.id;
}

function deleteDrivePermission_(fileId,permissionId){
  ...
  var res=driveApiFetch_(url,{method:'delete'});
  var code=res.getResponseCode();
...
function rememberTimedDriveAccess_(email,fileId,permissionId,kind){
  var normalized=String(email||'').trim().toLowerCase();
  var key=downloadAccessPropertyKey_(normalized,fileId);
  ...
  PropertiesService.getScriptProperties().setProperty(
    ...
    JSON.stringify({
      ...
      kind:String(kind||'DOWNLOAD').toUpperCase(),
      grantedAt:new Date().getTime()
    })
  );
}

function grantTimedDriveViewer_(email,fileId,kind){
  var normalized=String(email||'').trim().toLowerCase();
  ...
  // Confirm the file exists before permission work.
  var file=DriveApp.getFileById(id);

  var permissionId=createSilentDriveViewer_(id,normalized);
  rememberTimedDriveAccess_(normalized,id,permissionId,kind||'DOWNLOAD');

  // Existing trigger name is intentionally retained for backward compatibility.
  ensureTrialAccessCleanupTrigger_();

  return file.getUrl();
}

function ensureTrialAccessCleanupTrigger_(){
  try{
    var handler='cleanupExpiredTrialDriveAccess_';
...
/*
  Backward-compatible setup name.
  Run ONCE only if a cleanup trigger was never created earlier.
*/
function setupTrialAccessCleanup(){
  var installed=ensureTrialAccessCleanupTrigger_();
  var result=cleanupExpiredTrialDriveAccess_();
  return{
    installed:installed,
    accessHours:12,
    cleanup:result
...
/*
  BOTH TRIAL + PAID:
  Permission is removed after 12 hours.
  A paid customer can get another silent 12-hour window simply by logging
  into the Queshift dashboard again while the subscription is ACTIVE.
  A trial customer cannot renew because the trial record is already marked
  DOWNLOADED and repeat email/phone requests are blocked.
*/
function cleanupExpiredTrialDriveAccess_(){
  var props=PropertiesService.getScriptProperties();
  var all=props.getProperties();
  var prefix='QS_DOWNLOAD_ACCESS_';
  var nowMs=new Date().getTime();
  var maxAge=downloadAccessMinutes_()*60*1000;
  var checked=0,revoked=0,errors=0;
  ...
  Object.keys(all).forEach(function(key){
    ...
    var grantedAt=+item.grantedAt||0;
    if(!grantedAt||nowMs-grantedAt<maxAge)return;

    try{
      if(deleteDrivePermission_(item.fileId,item.permissionId)){
        revoked++;
        try{
          audit_(
            item.email||QS_ADMIN_EMAIL,
            'DOWNLOAD_DRIVE_ACCESS_REVOKED',
            JSON.stringify({
              fileId:item.fileId||'',
      ...
      errors++;
      // Keep property so the next trigger run can retry.
    }
  });

  return{checked:checked,revoked:revoked,errors:errors};
}
...
function trialGrantDownloadUrl_(email){
  /*
    TRIAL DOWNLOAD:
    - exact OTP-verified Gmail only
    - NO Google Drive "item shared" email
    - access expires after 12 hours
    - same email/phone cannot request another trial after download grant
  */
  var fileId=clean_(getSetting_('TRIAL_SOFTWARE_FILE_ID'));

  if(fileId){
    try{
      return grantTimedDriveViewer_(email,fileId,'TRIAL');
    }catch(_){
      return'';
    }
  }
  ...
  /*
    ...
    IMPORTANT: a public Mega/Drive URL cannot be force-expired by this script.
    For strict 12-hour security, keep Trial Download URL blank and use the
    Restricted Google Drive Trial File ID field.
  */
  var direct=clean_(getSetting_('TRIAL_DOWNLOAD_URL'));
  return direct||'';
}
...
function publicMediaUrl_(value){
  if(!value)return'';
...
function activationFindIssuedByMachine_(machineCode) {
  ...
  return rows_('ACTIVATIONS').filter(
    function(row) {
      return String(
        row.MACHINE_CODE || ''
      ...
      && String(
        row.STATUS || ''
      ).trim().toUpperCase() !== 'REVOKED';
    }
  )[0] || null;
}


function activationIssueCountForOrder_(orderId) {
  var key = String(
    orderId || ''
  ).trim().toUpperCase();
...
function activationUtcDay_(ymd) {
  var match = String(
    ymd || ''
  ).match(
    /^(\d{4})-(\d{2})-(\d{2})$/
...
function activationDaysBetween_(startYmd, endYmd) {
  return Math.round(
    (
      activationUtcDay_(endYmd)
      - activationUtcDay_(startYmd)
    ) / 86400000
  );
}


function activationOrderState_(orderId) {
  orderId = String(
    orderId || ''
  ).trim();
  ...
  if (!subscription) {
    throw new Error(
      'Subscription record was not found for this Order ID.'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'Website subscription start/expiry date is incomplete.'
    );
  }
  ...
  if (
    activationUtcDay_(today)
    > activationUtcDay_(expiry)
  ) {
    throw new Error(
      'This Queshift subscription has expired. Please renew the subscription before re-activation.'
    );
  }


  /*
  Re-activation must never restart the full plan period.

  If subscription is already running:
      license starts today.

  If subscription starts in future:
      license starts from website START_DATE.

  EXPIRY_DATE always remains exactly the website SUBSCRIPTIONS expiry.
  */
  var licenseStart =
    (
      activationUtcDay_(today)
      < activationUtcDay_(subscriptionStart)
    )
    ? subscriptionStart
    : today;
...
/*
SAME EXISTING ACTION:
    activationVerifyOrder

STEP 1:
    payload.lookupOnly = true

    Only Order ID is validated.
    No Machine Request Code is required.
    No Master Security Code is required.
    Website subscription dates/days are returned.

STEP 2:
    lookupOnly = false / omitted

    Master Security Code is required.
    New Machine Request Code is required.
    Same Order ID is ALLOWED again.
    Same Machine Request Code is NOT allowed again.
*/
function activationVerifyOrder_(payload, credential) {
  ...
  if (!machineCode) {
    throw new Error(
      'Machine Request Code is required.'
    );
  }


  /*
  Order ID is intentionally reusable.
  Only the exact Machine Request Code remains one-time.
  */
  if (
    activationFindIssuedByMachine_(
      machineCode
    )
  ) {
    throw new Error(
      'This Machine Request Code has already been used. '
      + 'Copy the current/new Machine Request Code from the client Queshift installation.'
    );
  }
...
The generated activation must match the CURRENT website subscription exactly.

Re-activation:
    same Order ID = allowed
    new Machine Request Code = required
    expiry = same website expiry
    days = only current remaining days

SUBSCRIPTIONS sheet is NOT modified here.
*/
function activationCommit_(payload, credential) {
  ...
  if (
    activationDate
    !== verified.licenseStartDate
  ) {
    throw new Error(
      'Activation start date changed during generation. Verify the Order ID again.'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'Subscription expiry changed during generation. Verify the Order ID again.'
    );
  }
  ...
  if (
    days
    !== Number(
      verified.remainingDays
    )
  ) {
    throw new Error(
      'Remaining days changed during generation. Verify the Order ID again.'
    );
  }
      ...
      JSON.stringify(
        {
          ...
          subscriptionStartDate:
            verified.subscriptionStartDate,
