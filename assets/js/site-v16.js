window.QUESHiFT_WEB_BUILD = "V16-MOTO-CACHE-KILL";
(function () {
  "use strict";
  ...
  const PUBLIC_CACHE_KEY = "qs_public_cache_v16";
  const PUBLIC_CACHE_TS_KEY = "qs_public_cache_v16_ts";
  const PUBLIC_CACHE_MAX_AGE = 24 * 60 * 60 * 1000; // emergency fallback only
  function getLocal() {
    try {
  ...
  function clearLegacyPublicCaches() {
    try {
      const keep = new Set([PUBLIC_CACHE_KEY, PUBLIC_CACHE_TS_KEY]);
      const remove = [];
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k && k.startsWith("qs_public_cache") && !keep.has(k)) remove.push(k);
      }
  ...
  function loadPublic() {
    // V16: hard network-first. Never import an older website snapshot.
    ...
    if (window.QSApi && QSApi.isConfigured()) {
      QSApi.get("publicData").then(remote => {
        ...
        // Only a cache created by V16 itself is allowed as fallback.
