(function () {
  ...
  async function action(name, payload) {
    note("Processing…");
    try {
      await QSApi.post(name, payload, QSApi.token());
      note("Completed successfully.");
      if (name === "approvePayment" || name === "rejectPayment") await loadPaymentsFast();
      else await load();
    } catch (error) {
  ...
  function renderPaymentsFast() {
    const body = document.querySelector("[data-admin-payments]");
    if (!body) return;

    body.innerHTML = (dashboard.payments||[]).map(x => `<tr>
      <td><b>${esc(x.orderId)}</b><br>${esc(x.name)}<br><small>${esc(x.phone)} · ${esc(x.state)}</small></td>
      <td>${esc(x.plan)}</td>
      <td>₹${Number(x.amount||0).toLocaleString("en-IN")}</td>
      <td><a href="${esc(x.screenshotUrl)}" target="_blank" rel="noopener">View Screenshot</a></td>
      <td><span class="badge ${x.status==='PENDING'?'warn':''}">${esc(x.status)}</span></td>
      <td>${x.status==='PENDING'
        ? `<select data-plan-for="${esc(x.orderId)}">${planOptions(x.plan)}</select>
           <input data-days-for="${esc(x.orderId)}" type="number" placeholder="Days">
           <button class="approve-btn" data-approve="${esc(x.orderId)}">Approve</button>
           <button class="reject-btn" data-reject="${esc(x.orderId)}">Reject</button>`
        : "—"}</td>
    </tr>`).join("") || '<tr><td colspan="6">No payment attempts.</td></tr>';

    document.querySelectorAll("[data-approve]").forEach(b => b.onclick = async () => {
      const orderId = b.dataset.approve;
      const planNode = document.querySelector(`[data-plan-for="${orderId}"]`);
      const daysNode = document.querySelector(`[data-days-for="${orderId}"]`);
      b.disabled = true;
      b.textContent = "Approving…";
      note("Approving payment, generating invoice and activating subscription…");
      try {
        await QSApi.post("approvePayment", {
          orderId,
          plan: planNode ? planNode.value : "",
          days: daysNode ? daysNode.value : ""
        }, QSApi.token());
        note("Payment approved successfully.");
        await loadPaymentsFast();
      } catch (error) {
        note(error.message, true);
        b.disabled = false;
    ...
    document.querySelectorAll("[data-reject]").forEach(b => b.onclick = async () => {
      b.disabled = true;
      note("Rejecting payment…");
      try {
        await QSApi.post("rejectPayment", { orderId:b.dataset.reject }, QSApi.token());
  ...
  async function loadPaymentsFast() {
    try {
      const fast = await QSApi.post("adminPaymentsFast", {}, QSApi.token());
      dashboard.plans = fast.plans || [];
      dashboard.payments = fast.payments || [];
      dashboard.counts = Object.assign({}, dashboard.counts || {}, {
        pendingPayments: fast.pendingPayments || 0
      });
      renderPaymentsFast();
      return true;
    } catch (error) {
      note("Payment list error: " + error.message, true);
      return false;
    }
  }
  ...
  async function load() {
    try {
      dashboard = await QSApi.post("adminDashboard", {}, QSApi.token());
      render();
      note("Dashboard is up to date.");
    } catch (error) {
      note("Full dashboard: " + error.message, true);
      if (/token|admin|author/i.test(error.message)) {
        setTimeout(() => { QSApi.clearSession(); location.href = "admin.html"; }, 1200);
      }
    }
  }
  ...
  async function startAdmin() {
    setupUploadGuidance();
    note("Loading payments…");
    const paymentsReady = await loadPaymentsFast();
    if (paymentsReady) note("Payments ready. Loading remaining dashboard…");
    // Full dashboard is secondary; Payments remain usable even if this is slow.
    load();
  }
