(function () {
  ...
  async function post(action, payload, token) {
    ...
    const adminActions = /^(adminDashboard|adminPaymentsFast|saveSettings|saveSocial|saveBanner|savePartner|saveBrand|saveVideo|saveBlog|saveHelpArticle|savePlan|saveSoftwareFile|deleteContent|approvePayment|rejectPayment|reviewAction|enquiryAction)$/;
    const timeoutMs =
      action === "approvePayment" ? 120000 :
      action === "adminDashboard" ? 90000 :
      action === "adminPaymentsFast" ? 45000 :
      action === "adminLogin" || action === "login" ? 65000 :
      (adminActions.test(action) ? 60000 : 45000);
